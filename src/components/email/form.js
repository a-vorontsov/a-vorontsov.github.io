...
export default class EmailForm extends React.Component {
    ...
    displayResponseStatus(response) {
        switch (response) {
            case 2:
                this.setState({response: {success: "Message received successfully. Thank you."}});
                break;
            case 4:
                this.setState({response: {error: "Error sending message. Please check your details or try again later."}})
                break;
            case 5:
                this.setState({response: {error: "Server error. Please try again later."}})
                break;
        }
        window.setTimeout(() => {
        ...
        return (
            <div>
                ...
                </div>
                {
                    (() => {
                        if (!!success) {
                            return <span className={styles}><span className="success">{success}</span></span>
                        } else if (!!error) {
